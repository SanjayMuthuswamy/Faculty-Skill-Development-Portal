...
export default function AdminReports() {
    ...
    const deptStats = useMemo(() => {
        if (!rawDeptStats) return [];
        return rawDeptStats.map(d => ({
            dept: d.department,
            planAdoption: Math.round(d.plan_adoption_rate),
            avgPracticeScore: Math.round(d.avg_accuracy),
            programs: d.total_enrollments,
            facultyCount: d.faculty_count,
            verifiedRate: d.verified_skills_rate
        }));
    }, [rawDeptStats]);

    const globalKPIs = useMemo(() => {
        if (deptStats.length === 0) return { adoption: 0, accuracy: 0, verification: 0 };

        const totalFaculty = deptStats.reduce((acc, curr) => acc + curr.facultyCount, 0);
        if (totalFaculty === 0) return { adoption: 0, accuracy: 0, verification: 0 };
